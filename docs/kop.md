---
download: "https://github.com/streamnative/kop/releases/download/v2.8.0.5/pulsar-protocol-handler-kafka-2.8.0.5.nar"
...
1. Download KoP protocol handler, or build the `./kafka-impl/target/pulsar-protocol-handler-kafka-2.8.0.5.nar` file, and then copy it to your Pulsar `protocols` directory.
...
3. Get the `.nar` file in the following directory and copy it your Pulsar `protocols` directory. You need to create the `protocols` folder in Pulsar if it's the first time you use protocol handlers.
    ...
    ```bash
    ./kafka-impl/target/pulsar-protocol-handler-kafka-2.8.0.5.nar
    ```
